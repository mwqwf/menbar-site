...
/* ============================================================
   شريط الدعوة إلى التطبيق (صفحة الدرس المُشارَكة).

   لماذا هنا؟ الرابط الذي يشاركه المستخدم من التطبيق يفتح صفحة الدرس، ومن
   لا تطبيق عنده يقف عندها — وهي أنسب لحظة لدعوته. و`app.js` يرسم شريطاً
   مطابقاً (`appBannerHtml`) بعد أن يستبدل `#app`، فيبقى الشريط **واحداً**
   دائماً: هذا قبل وصول الـJS، وذاك بعده.

   بلا تصميم جديد: يستعمل صنف `.app-banner` الموجود في app.css.
   ============================================================ */
function inviteBar() {
  return '<div class="app-banner">' +
    '<p>استمع لبقيّة السلسلة في تطبيق ' + escapeHtml(BRAND) + '</p>' +
    '<a href="' + PLAY_URL + '" target="_blank" rel="noopener">حمّل التطبيق</a>' +
    '</div>';
}

module.exports = { shell, BRAND, PLAY_URL, inviteBar };
